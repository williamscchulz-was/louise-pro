...
- **Versão atual**: v11.9.130 (app) / routine-engine v2.2.4
- **Orb "nebulosa líquida" no miolo do Ring (v11.9.130):** escolha do William ("Opcao B do mockup" `orb-premium.html`, referência Thinking Orbs/Antalík). **Substitui** a nebulosa v11.9.82 (`ring-neb`/`ring-neb2` + poeira `ring-dust-*` — elementos E classes CSS removidos; `ring-pulse` do timer mantido). Esfera `sz*0.56` com `overflow:hidden` + rim light no topo + glow externo, contendo 3 blobs (`.orb-liq` + `.orb-liqA/B/C`, ciclos 17/21/19s alternate, transform-only, blur 12px) — paleta por estado computada em JS (dia lilás/ciano/rosa · soneca roxo ×0.75 · noite índigo ×0.55 via `isBedtime`), interpolada só no box-shadow (`transition`). **No banho o orb NÃO renderiza** (`!isBath`) — decisão consciente: a água do miolo (v11.9.115) continua protagonista, evita duas bordas circulares brigando. zzz da soneca renderiza DEPOIS do orb (fica por cima, dentro da esfera). Sempre visível (inclusive ocioso — é upgrade de elemento que já era always-on); se o William preferir só-com-timer, é 1 condição. CSS puro, zero WebGL/canvas/dependência.
