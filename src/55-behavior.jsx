...
const BehaviorPage = React.memo(function BehaviorPage({entries,birthDate,onBack,lang}){
  ...
  function BehaviorChart({label,emoji,unit,field,color,fmtVal,band,idx}){
    ...
    return(<div className="home-rise-in" style={{background:"linear-gradient(180deg,rgba(22,28,60,0.55),rgba(20,26,60,0.32))",border:`1px solid ${T.gBSoft}`,borderRadius:18,padding:"15px 16px",marginBottom:13,boxShadow:T.cardShadow,animationDelay:`${(idx||0)*70}ms`}}>
      ...
      <div style={{position:"relative",height:13,marginTop:4}}>
        {tickIdx.map(i=><span key={i} style={{position:"absolute",left:`${(x(i)/W)*100}%`,transform:"translateX(-50%)",fontSize:T.fXS,color:T.label,fontWeight:600,letterSpacing:0.3}}>{dLbl(pts[i].endDate)}</span>)}
      </div>
    </div>);
  }
  ...
  return(<div className="page-switch" style={{position:"fixed",inset:0,zIndex:200,maxWidth:480,margin:"0 auto",background:T.pageBg,overflowY:"auto",overflowX:"hidden",overscrollBehavior:"none",WebkitOverflowScrolling:"touch",paddingTop:"env(safe-area-inset-top)",paddingBottom:"calc(100px + env(safe-area-inset-bottom))"}}>
    ...
    {weekly.length<2?(
      <div style={{padding:"60px 24px 20px",textAlign:"center"}}>
        ...
        <div style={{fontSize:T.fLG,fontWeight:700,color:T.heading,letterSpacing:-0.2,marginBottom:6}}>{weekly.length===1?(_lang==="en"?"Almost there":"Quase lá"):(_lang==="en"?"Not enough weeks yet":"Poucas semanas ainda")}</div>
        <div style={{fontSize:T.fMD,color:T.sub,lineHeight:1.55,maxWidth:270,margin:"0 auto"}}>{weekly.length===1?(_lang==="en"?"One more week of logging and the curves unlock.":"Mais uma semana de registros e as curvas destravam."):(_lang==="en"?"Keep logging for a couple of weeks and the behavior curves will appear here.":"Continue registrando por algumas semanas e as curvas de comportamento aparecem aqui.")}</div>
        {weekly.length===1&&<div style={{display:"flex",gap:6,justifyContent:"center",marginTop:14}}>
          <div style={{width:20,height:5,borderRadius:3,background:T.lilac}}/>
          <div style={{width:20,height:5,borderRadius:3,background:"rgba(139,124,246,0.18)"}}/>
        </div>}
      </div>
    ):(<>
      ...
      <div style={{padding:"6px 0 20px",fontSize:T.fXS,color:T.label,textAlign:"center",lineHeight:1.6}}>
