...
function suggestQuickActions(allEntries,todayList,nowMin,routine){
  ...
  const top=CANDS.slice().sort((a,b)=>score[b]-score[a]).slice(0,3);
  const hasWakeup=(todayList||[]).some(e=>e.type==="wakeup");
  if(!hasWakeup&&h>=5&&h<11){top.pop();top.unshift("wakeup")}
  // v11.9.93: posição CANÔNICA — o score escolhe QUAIS 3, a ordem fixa decide ONDE
  // (alimentação à esquerda, sono no centro, cuidado à direita). Memória muscular >
  // leitura de label: mis-tap em Soneca/Sono inicia timer, não pode trocar de lugar.
  const SLOT_ORDER={wakeup:0,bottle:1,nursing:2,nap:3,sleep:4,diaper:5,bath:6,medicine:7};
  return top.sort((a,b)=>(SLOT_ORDER[a]??9)-(SLOT_ORDER[b]??9));
}
...
function sleepReport(entries,days){
  ...
  for(let i=days-1;i>=0;i--){
    ...
    rows.push({date,
      wake,night,bedtime:bed?bed.start:null,naps:dayNaps,napCount:dayNaps.length,napTotalMin,
      windows,maxWindowMin:windows.reduce((m,w)=>w.min>m?w.min:m,0),
