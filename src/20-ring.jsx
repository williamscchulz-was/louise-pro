const Ring = React.memo(function Ring({activeTimer,napSug,tick,recentEvents,lang,showOrbit,screenLockActive}){
  ...
  const isNap=!!activeTimer&&activeTimer.type==="nap";
  const isTum=!!activeTimer&&activeTimer.type==="tummytime";
  ...
  // v11.9.148: comida é um ponto âmbar na hora real, com o alimento no tooltip.
  const dotEvs=evs.filter(e=>e.type==="bottle"||e.type==="nursing"||e.type==="food"||e.type==="diaper"||e.type==="medicine"||e.type==="tummytime"||e.type==="bath"||e.type==="temperature");
    ...
    <div style={{position:"relative",width:sz,height:sz}}>
      ...
      {timerArc&&<>
        {activeTimer.type==="bath"
          ...
          : isNap
          ? <div className="sheep-bob" style={{position:"absolute",left:timerArc.mx-17,top:timerArc.my-14,zIndex:4,filter:"drop-shadow(0 2px 5px rgba(0,0,0,0.45))"}}><svg viewBox="0 0 44 36" width="34" height="28"><ellipse cx="17" cy="19" rx="5.5" ry="5" fill="#f4f1ea"/><ellipse cx="24" cy="15" rx="6" ry="5.5" fill="#f4f1ea"/><ellipse cx="27" cy="22" rx="6.5" ry="6" fill="#f4f1ea"/><ellipse cx="19" cy="24" rx="6" ry="5.5" fill="#f4f1ea"/><ellipse cx="22" cy="20" rx="9" ry="7.5" fill="#fdfcf9"/><ellipse cx="10" cy="21" rx="6.5" ry="6" fill="#e8c9a0"/><path d="M4 17 Q1 21 5 24" stroke="#d9b183" strokeWidth="1.8" fill="none" strokeLinecap="round"/><path d="M6.5 19 q2 -1.5 3.5 0" stroke="#5c4632" strokeWidth="1.2" fill="none" strokeLinecap="round"/><path d="M12.5 19 q2 -1.5 3.5 0" stroke="#5c4632" strokeWidth="1.2" fill="none" strokeLinecap="round"/><path d="M9 25 q1.5 1.3 3 0" stroke="#c48f5e" strokeWidth="1.1" fill="none" strokeLinecap="round"/></svg></div>
          : isTum
          ? <div className="rocket-bob" style={{position:"absolute",left:timerArc.mx-13,top:timerArc.my-17,zIndex:4,filter:"drop-shadow(0 2px 5px rgba(0,0,0,0.45))"}}><svg viewBox="0 0 30 38" width="26" height="33"><path d="M15 2 C20 8 21 16 20 24 L10 24 C9 16 10 8 15 2 Z" fill="#e8e6f5"/><path d="M15 2 C17.8 5.4 19.3 9 19.8 12 L10.2 12 C10.7 9 12.2 5.4 15 2 Z" fill="#f472b6"/><circle cx="15" cy="16" r="3.2" fill="#22304f" stroke="#9db4d8" strokeWidth="1.2"/><path d="M10 20 Q4 24 5 31 L10 26 Z" fill="#f472b6"/><path d="M20 20 Q26 24 25 31 L20 26 Z" fill="#f472b6"/><g className="rocket-flame"><path d="M12 25 Q15 36 18 25 Q15 29 12 25" fill="#fbbf24"/><path d="M13.5 25 Q15 31 16.5 25" fill="#fde68a"/></g></svg></div>
          : <div style={{position:"absolute",left:timerArc.mx-sleepSz/2,top:timerArc.my-sleepSz/2,width:sleepSz,height:sleepSz,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",zIndex:4,filter:`drop-shadow(0 0 10px ${timerArc.col}aa)`,animation:"glowBreathe 2s ease-in-out infinite"}}><Icon name={activeTimer.type==="tummytime"?"baby":"bed"} size={sleepIconSz+2} color={timerArc.col}/></div>}
        {!isBath&&!isNap&&!isTum&&<div style={{position:"absolute",left:timerArc.tipX-5,top:timerArc.tipY-5,width:10,height:10,borderRadius:"50%",background:timerArc.col,boxShadow:`0 0 7px ${timerArc.col}aa, 0 0 16px ${timerArc.col}55`,animation:"cometTip 1.5s ease-in-out infinite",zIndex:5}}/>}
      </>}
      {dots.map((gr,i)=>{const d=gr.primary,c=d.cfg.color,isGrp=gr.items.length>1,active=tip===i;const isLast=i===lastDotIdx&&!activeTimer;const lines=gr.items.map(it=>{let l=TL(it.ev.type);if(it.ev.ml)l+=` \u00b7 ${it.ev.ml}ml`;else if(it.ev.type==="food"&&it.ev.name)l+=` \u00b7 ${it.ev.name}`;else if(it.ev.subtype)l+=` \u00b7 ${({wet:L("wet"),dirty:L("dirty"),both:L("both")})[it.ev.subtype]||""}`;return`${l} \u00b7 ${it.ev.time}`});return(<div key={d.ev.id||i} style={{position:"absolute",left:d.x-dotSz/2,top:d.y-dotSz/2}}>
      ...
      {/* v11.9.82: nebulosa + micro-interações no miolo — preenche o vazio atrás do tempo.
          ...
          (v11.9.131: restaurada — o orb da v11.9.130 foi removido a pedido do William.) */}
      ...
      {/* v11.9.119: "zzz" subindo no miolo durante a soneca — mesmo espírito da água do
          banho (sem zIndex, DOM antes do texto → texto por cima). */}
      {isNap&&<div style={{position:"absolute",top:"50%",left:"50%",width:sz*0.4,height:sz*0.4,transform:"translate(-50%,-50%)",pointerEvents:"none"}}>
        {[{l:44,fs:13,d:0},{l:54,fs:17,d:1.1},{l:48,fs:21,d:2.2}].map((z,i)=><div key={"zzz"+i} className="zzz-float" style={{position:"absolute",left:z.l+"%",bottom:8,fontWeight:800,fontFamily:"Georgia,serif",color:"#e9d5ff",fontSize:z.fs,animationDelay:z.d+"s"}}>{i===2?"Z":"z"}</div>)}
      </div>}
      ...
      {/* v11.9.134: o número central NÃO pode ter background-clip:text E filter no MESMO
          elemento — no WebKit (iPhone) isso promove o texto a uma camada com fundo opaco que
          TAPA a nebulosa atrás, criando um retângulo sem névoa em volta do número (reportado
          pelo William com print). O brilho mora num wrapper; o gradiente fica no span de dentro. */}
      <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}><span style={{fontSize:T.fMD,color:isBedtime?"#c084fc":T.sub,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase",display:"flex",alignItems:"center",gap:5,animation:isBedtime?"bedtimeSubBreath 5s ease-in-out infinite":"none"}}>{isBedtime&&<Icon name="bed" size={13} color="#c084fc"/>}{sub}{screenLockActive&&<span title={lang==="en"?"Screen on":"Tela ligada"} style={{display:"inline-flex",alignItems:"center",gap:3,marginLeft:4,padding:"2px 6px",borderRadius:8,background:"rgba(251,191,36,0.12)",border:"1px solid rgba(251,191,36,0.3)"}}><span style={{width:5,height:5,borderRadius:"50%",background:"#fbbf24",boxShadow:"0 0 6px #fbbf24",animation:"pulse 1.5s ease-in-out infinite"}}/><span style={{fontSize:T.fXS,fontWeight:700,color:"#fbbf24",letterSpacing:0.5,textTransform:"uppercase"}}>{lang==="en"?"On":"On"}</span></span>}</span><span style={{marginTop:4,display:"block",filter:isBedtime?"drop-shadow(0 0 18px rgba(167,139,250,0.55))":(hasProgress?`drop-shadow(0 0 20px ${col1}55)`:"drop-shadow(0 0 24px rgba(167,139,250,0.32))")}}><span style={{fontSize:bigFont,fontWeight:800,letterSpacing:-1.8,background:isBedtime?"linear-gradient(110deg, #7c3aed 0%, #8b5cf6 25%, #e9d5ff 48%, #ffffff 50%, #e9d5ff 52%, #8b5cf6 75%, #7c3aed 100%)":(hasProgress?`linear-gradient(135deg,${col2},${col1})`:`linear-gradient(180deg,#ffffff 0%,#a78bfa 95%)`),backgroundSize:isBedtime?"300% 100%":"100% 100%",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",animation:isBedtime?"bedtimeShimmer 10s linear infinite":"none",fontVariantNumeric:"tabular-nums"}}>{lbl}</span></span>{napSug&&napSug.predictedTime&&!activeTimer&&<span style={{fontSize:T.fSM,fontWeight:600,color:col1,opacity:0.7,marginTop:2}}>{lang==="en"?"Nap":"Nap"} {napSug.napPos||""} ~{napSug.predictedTime}</span>}</div>
