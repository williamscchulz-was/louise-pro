const Ring = React.memo(function Ring({activeTimer,napSug,tick,recentEvents,lang,showOrbit,screenLockActive}){
  ...
  let timerArc=null;
  if(activeTimer&&(activeTimer.type==="nap"||activeTimer.type==="tummytime")){
    ...
    if(startMins>=dayStart){
      ...
      if(a2>a1){const midAng=(a1+a2)/2;const midRad=midAng*Math.PI/180;const tipRad=a2*Math.PI/180;timerArc={path:descArc(a1,a2),col:activeTimer.type==="tummytime"?T.amber:T.purple,mx:cx+R*Math.cos(midRad),my:cy+R*Math.sin(midRad),tipX:cx+R*Math.cos(tipRad),tipY:cy+R*Math.sin(tipRad)}}
    }
  }
  ...
  return(<div style={{position:"relative",width:sz,margin:"0 auto"}}>
    <div style={{position:"relative",width:sz,height:sz}}>
      ...
      {timerArc&&<>
        ...
        <div style={{position:"absolute",left:timerArc.tipX-5,top:timerArc.tipY-5,width:10,height:10,borderRadius:"50%",background:timerArc.col,boxShadow:`0 0 7px ${timerArc.col}aa, 0 0 16px ${timerArc.col}55`,animation:"cometTip 1.5s ease-in-out infinite",zIndex:5}}/>
      </>}
      ...
      <div className="ring-neb" style={{position:"absolute",top:"50%",left:"50%",width:sz*0.62,height:sz*0.4,borderRadius:"50%",background:isBedtime?"radial-gradient(ellipse at 50% 50%,rgba(139,124,246,0.30),rgba(124,58,237,0.13) 45%,transparent 72%)":"radial-gradient(ellipse at 50% 50%,rgba(139,124,246,0.15),rgba(124,58,237,0.05) 48%,transparent 73%)",filter:"blur(16px)",pointerEvents:"none"}}/>
      <div className="ring-neb2" style={{position:"absolute",top:"50%",left:"50%",width:sz*0.46,height:sz*0.3,borderRadius:"50%",background:`radial-gradient(ellipse at 50% 50%,rgba(196,181,253,${isBedtime?0.18:0.09}),transparent 70%)`,filter:"blur(11px)",pointerEvents:"none"}}/>
