...
function ReportSection({entries,babyName,onOpenReport}){
  const[msg,setMsg]=useState("");
  const[busy,setBusy]=useState(false);
  ...
  return(<div style={{marginTop:12,padding:"16px 18px",borderRadius:14,background:"rgba(22,28,60,0.4)",border:`1px solid ${T.gB}`,boxShadow:T.insetTop}}>
    ...
    <div style={{fontSize:T.fSM,color:T.sub,lineHeight:1.5,marginBottom:12}}>{_lang==="en"?"A document with averages, a 30-day sleep map and a day-by-day table — wake-up time, naps with durations, awake windows, feeds and night sleep. Save it, print it or send it straight on WhatsApp.":"Um documento com médias, o mapa de sono dos 30 dias e a tabela dia a dia — horário de acordar, sonecas com duração, janelas acordada, mamadas e sono noturno. Dá pra salvar, imprimir ou mandar direto no WhatsApp."}</div>
    <div style={{display:"flex",gap:8}}>
      <button onClick={onOpenReport} disabled={busy} style={{flex:1,padding:"11px 12px",borderRadius:11,background:busy?"rgba(139,124,246,0.1)":"linear-gradient(180deg,rgba(139,124,246,0.22),rgba(124,58,237,0.12))",border:"1px solid rgba(139,124,246,0.35)",color:T.lilac,fontSize:T.fMD,fontWeight:700,letterSpacing:-0.1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,opacity:busy?0.6:1}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/></svg>
        {_lang==="en"?"Open report":"Ver relatório"}
      </button>
