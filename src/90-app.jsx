function App(){
  ...
  const _VALID_PAGES = ["home","stats","history","growth"];
  ...
  const addEntry=useCallback(async e=>{
    ...
    let tmsg=`${TL(e.type)||e.type} ${wasEdit?L("updated"):L("registered")}`;
    if(!wasEdit){
  ...
  const deleteEntry=useCallback(async id=>{const entry=entries.find(e=>e.id===id);await FB.delEntry(id);showToast(`${TL(entry?.type)||""} ${L("removed")}`,async()=>{if(entry)await FB.addEntry(entry)})},[entries]);
  ...
  const routineState=useMemo(()=>{
    ...
    // v11.9.88: estágios pós-banho — mamadeira pós-banho + Floripa + vitamina D (default
    // banho+15min, configuráveis nos Ajustes). Remédios DIÁRIOS casam por NOME em qualquer
    // hora do dia (deu hoje = feito; o horário do slot só ordena o "próximo"). A mamadeira
    // pós-banho casa por JANELA assimétrica (45 antes / 90 depois) — mamadeira acontece ~7x/dia,
    // não dá pra casar qualquer uma; a janela curta pra trás evita pegar a mamada pré-banho.
    const medToday=re=>todayE.find(e=>e.type==="medicine"&&e.name&&re.test(e.name));
    if(r.postBathBottleTime){const tMin=adj(toMinutes(r.postBathBottleTime),"pb");slots.push({key:"pbbottle",type:"pbbottle",icon:"bottle",col:T.green,target:minToTime(tMin),targetMin:tMin,tolMin:60,match:findMatch(["bottle"],tMin,45,90),lbl:_lang==="en"?"Post-bath bottle":"Mamad. pós-banho",chip:_lang==="en"?"Bottle":"Mamad."})}
    if(r.floripaTime){const tMin=adj(toMinutes(r.floripaTime),"pb");slots.push({key:"floripa",type:"floripa",icon:"pill",col:T.amber,target:minToTime(tMin),targetMin:tMin,tolMin:60,match:medToday(/florip/i),lbl:"Floripa",chip:"Floripa"})}
    if(r.vitdTime){const tMin=adj(toMinutes(r.vitdTime),"pb");slots.push({key:"vitd",type:"vitd",icon:"droplet",col:T.amber,target:minToTime(tMin),targetMin:tMin,tolMin:60,match:medToday(/vit(amina)?\s*\.?\s*d/i),lbl:_lang==="en"?"Vitamin D":"Vitamina D",chip:"Vit D"})}
  ...
  return(<div style={{background:"transparent",height:"var(--phys-h, 100dvh)",overflowY:"auto",overflowX:"hidden",WebkitOverflowScrolling:"touch",overscrollBehavior:"none",color:T.text,maxWidth:480,margin:"0 auto",position:"relative"}}>
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      {isSwipePage?<div style={{overflow:"hidden",width:"100%",touchAction:"pan-y"}}>
        ...
        {activeTimer&&activeTimer.type==="sleep"&&<div style={{display:"flex",justifyContent:"center",marginBottom:10,padding:"0 20px"}}>
          <button onClick={toggleNightWake} className={activeTimer.nightWake?"":"nw-pill-pulse"} style={{padding:"7px 14px",borderRadius:20,background:activeTimer.nightWake?"linear-gradient(135deg,rgba(251,146,60,0.22),rgba(251,191,36,0.10))":"linear-gradient(135deg,rgba(251,191,36,0.18),rgba(251,191,36,0.06))",border:`1px solid ${activeTimer.nightWake?"rgba(251,146,60,0.60)":"rgba(251,191,36,0.50)"}`,display:"flex",alignItems:"center",gap:7,fontSize:T.fSM,fontWeight:700,color:activeTimer.nightWake?"#fb923c":"#fbbf24",letterSpacing:0.5,textTransform:"uppercase",cursor:"pointer",transition:"background .25s ease, border-color .25s ease, color .25s ease"}}>
        ...
        {!activeTimer&&<div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:8,padding:"0 20px",marginBottom:12}}>
          {quickSuggestions.map(tp=>{
            ...
            const SHORT={bottle:_lang==="en"?"Bottle":"Mamad.",nursing:_lang==="en"?"Nurse":"Amam.",diaper:_lang==="en"?"Diaper":"Fralda",sleep:_lang==="en"?"Sleep":"Sono",nap:_lang==="en"?"Nap":"Soneca",bath:_lang==="en"?"Bath":"Banho",medicine:_lang==="en"?"Medicine":"Medic."};
            const lbl=SHORT[tp]||(cfg.label?cfg.label[_lang]:tp);
            const act=tp==="nursing"?(()=>setShowNursingPicker(true)):(tp==="sleep"||tp==="nap"||tp==="tummytime")?(()=>startTimer(tp)):(()=>{setFormType(tp);setShowAdd(false)});
        ...
        <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:6,padding:"0 20px",marginBottom:14}}>{[{i:"bottle",v:`${totalMl}`,u:"ml",c:T.green,t:["bottle"],tap:"bottle",l:_lang==="en"?"Bottle":"Mamad.",num:totalMl},{i:"moon",v:fmtDur(totalSleep),c:T.purple,t:["sleep","nap"],tap:"nap",l:_lang==="en"?"Sleep":"Sono"},{i:"pill",v:`${totalSimet}`,u:"x",c:T.amber,t:["medicine"],tap:null,l:"Simeticona",filter:e=>e.type==="medicine"&&e.name&&/simet/i.test(e.name),num:totalSimet},{i:"pill",v:`${totalTylenol}`,u:"x",c:T.red,t:["medicine"],tap:null,l:"Tylenol",filter:e=>e.type==="medicine"&&e.name&&/tylenol|paraceta/i.test(e.name),num:totalTylenol},{i:"diaper",v:`${totalDiapers}`,c:T.pink,t:["diaper"],tap:"diaper",l:_lang==="en"?"Diaper":"Fralda",num:totalDiapers,span2:true}].map((p,idx)=>{const last=entries.find(e=>p.filter?p.filter(e):p.t.includes(e.type));let ago=null;if(last){if(p.t.includes("sleep")&&last.durationMin){const endMs=new Date(`${last.date}T${last.time}`).getTime()+last.durationMin*60000;const endD=new Date(endMs);const ed=`${endD.getFullYear()}-${String(endD.getMonth()+1).padStart(2,"0")}-${String(endD.getDate()).padStart(2,"0")}`;const et=`${String(endD.getHours()).padStart(2,"0")}:${String(endD.getMinutes()).padStart(2,"0")}`;ago=timeSince(ed,et)}else{ago=timeSince(last.date,last.time)}}
    ...
    {showProfile?<nav style={{position:"fixed",bottom:"calc(8px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",width:"auto",maxWidth:"none",opacity:1,display:"flex",alignItems:"center",gap:8,padding:"0",pointerEvents:"auto",background:"transparent",border:"none",boxShadow:"none",zIndex:250}}>
      ...
      {[{icon:"chart",label:L("stats"),active:page==="stats"||page==="growth",action:()=>goTo("stats")},{icon:"gear",label:L("settings"),active:false,action:()=>setShowProfile(true)}].map(n=><button key={n.icon} aria-label={n.label} className="navbtn" onClick={()=>{if(window.Haptic&&Haptic.light)Haptic.light();n.action()}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:46,height:44,borderRadius:18,background:n.active?"rgba(139,124,246,0.22)":"transparent",border:"none",padding:0}}><Icon name={n.icon} size={n.active?18:17} color={n.active?"#ffffff":T.dim}/></button>)}
    </nav>}
